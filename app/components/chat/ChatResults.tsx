...
const CustomMessageContainer: React.FC<CustomMessageContainerProps> = ({
  Icon,
  text,
}) => (
...
export const ChatResults: React.FC<ChatResultsProps> = ({
  ...
  const { data, isLoading, error, isRefetching } = useQuery(
    getChatSimiliarityQueryOptions(query),
  );
  ...
  if (isLoading)
    // return <CustomMessageContainer Icon={LoaderCircle} text="Cargando" />;
    return (
      <div className="text-neutral-400 text-center flex justify-center items-center gap-2 mb-4 mt-5">
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-8 h-8 text-gray-200 animate-spin dark:text-neutral-500 fill-neutral-950"
            viewBox="0 0 100 101"
            fill="none"
      ...
      {data && data.length ? (
        <div className="flex flex-col gap-2">
          {data.map((item) => (
            <div
              key={item.id}
              className="flex items-center bg-neutral-700 rounded-md hover:bg-neutral-600 h-auto px-4 py-3 gap-3 cursor-pointer"
