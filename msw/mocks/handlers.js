...
const companyData = {
  mision: "Nuestra misión es innovar y liderar proporcionando las mejores soluciones tecnológicas.",
  vision: "Ser el líder global en innovación tecnológica y satisfacción del cliente.",
  historia: "Nuestra empresa fue fundada en 2001 con el objetivo de revolucionar la industria tecnologica, y desde entonces hemos crecido hasta convertirnos en un referente en el sector.",
  valores: "Nuestros valores incluyen integridad, innovación, trabajo en equipo, y compromiso con la excelencia.",
  servicios: "Ofrecemos una amplia gama de servicios, incluyendo desarrollo de software personalizado, consultoria en IA, soluciones en la nube, y soporte tecnico.",
  proyectos: "Tenemos varios proyectos, entre ellos: Proyecto Alfa, que desarrolla una nueva plataforma de IA para empresas, y Proyecto Beta, que crea una solución segura en la nube para clientes corporativos.",
  organigrama: "El organigrama de la empresa es el siguiente: CEO: Juan Pérez, CTO: Ana Gómez, COO: Carlos López, VP de Ingeniería: Laura Martínez, VP de Ventas: David García.",
  contacto: "Puedes contactarnos por correo electronico a contacto@empresa.com o llamarnos al +34 912 345 678."
};
...
export const handlers = [
  ...
  http.post('https://api.example.com/message', async ({ request }) => {
    ...
    if (/\bgracias\b|\bthank you\b/.test(message)) response = defaultAnswers.agradecimiento;
    if (/\badios\b|\bhasta luego\b/.test(message)) response = defaultAnswers.despedida;
    ...
    if (/\borgani?grama\b|\borganigrama\b|\bequipo\b/.test(message)) response = companyData.organigrama;
    if (/\bcontacto\b|\bcual es el contacto\b/.test(message)) response = companyData.contacto;
